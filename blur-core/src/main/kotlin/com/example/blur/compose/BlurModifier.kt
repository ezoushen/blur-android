package com.example.blur.compose
 ...
 * effect, use [BlurSurface] which uses the View-based [com.example.blur.view.BlurView]
...
@Deprecated(
    ...
    replaceWith = ReplaceWith("BlurSurface(config = config) { content }", "com.example.blur.compose.BlurSurface")
)
@Stable
...
@Deprecated(
    message = "This modifier blurs the composable's content, NOT the background. Use BlurSurface for true blur-behind effect.",
    replaceWith = ReplaceWith("BlurSurface(radius = radius, overlayColor = overlayColor) { content }", "com.example.blur.compose.BlurSurface")
)
@Stable
fun Modifier.blurBehind(
    radius: Float = 16f,
